    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    if (userManager.FindUser(model.Username))
                    {
                        var oldUser = userManager.LoginUser(model.Username, model.Password);
        ...
        public async Task<ActionResult> RegisterEmployeeUser(RegisterEmployeeViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    if (usrMgr.FindUser(model.UserName))
                    {
                        return RedirectToAction("Register", "Account");
                    }
                    else
                    {
                        var employee = new DataObjects.WPFUser()
                        {
                            UserName = model.UserName
                        };
                        if (usrMgr.AddUser(employee.UserName))
                        {
                            var employeeID = usrMgr.RetrieveUserIDFromUsername(model.UserName);
                            var user = new ApplicationUser
                            {
                                UserID = employeeID,
                                UserName = model.UserName,
